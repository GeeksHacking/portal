...
<template>
  <UDashboardPanel id="dashboard-management">
    <template #header>
      <UDashboardNavbar title="Manage events">
        <template #leading>
      ...
      <div class="mx-auto w-full max-w-7xl space-y-6">
        <section class="space-y-4">
          <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div class="max-w-3xl space-y-3">
              <div class="flex flex-wrap items-center gap-2">
                ...
                <span class="text-xs text-muted">
                  {{ user?.gitHubLogin ? `Signed in as @${user.gitHubLogin}` : 'Community dashboard' }}
                </span>
              </div>
              <div>
                <h1 class="text-2xl font-semibold tracking-tight text-default sm:text-3xl">
                  Manage GeeksHacking events.
                </h1>
                <p class="mt-2 text-sm leading-6 text-muted">
                  Create, publish, and maintain hackathons and standalone workshops from one organizer workspace.
                </p>
              </div>
            </div>
            ...
            <div
              v-if="user?.isRoot"
              class="flex flex-col gap-2 sm:flex-row lg:justify-end"
            >
              <UButton
                icon="i-lucide-plus"
        ...
        <section class="rounded-lg border border-default bg-default p-3">
          <div class="grid gap-3 xl:grid-cols-[minmax(260px,1fr)_auto] xl:items-center">
            ...
            <div class="flex flex-col gap-2 lg:flex-row lg:items-center xl:justify-end">
              <div class="flex gap-1 rounded-md bg-elevated p-1">
                <UButton
              ...
              <div class="flex gap-1 overflow-x-auto rounded-md bg-elevated p-1">
                <UButton
                  v-for="filter in opportunityFilters"
                  :key="filter.value"
                  size="sm"
                  :color="activeOpportunity === filter.value ? 'primary' : 'neutral'"
                  :variant="activeOpportunity === filter.value ? 'soft' : 'ghost'"
                  class="shrink-0"
