...
function openEditResource(resource: GeeksHackingPortalApiEndpointsOrganizersHackathonResourcesListResponseResponseResource) {
...
async function saveResource() {
  if (!standaloneWorkshopId.value || !resourceForm.value.name.trim())
    return

  const data = {
    name: resourceForm.value.name.trim(),
    description: resourceForm.value.description.trim() || null,
    redemptionStmt: resourceForm.value.redemptionStmt.trim() || undefined,
...
async function deleteResource(resource: GeeksHackingPortalApiEndpointsOrganizersHackathonResourcesListResponseResponseResource) {
  if (!resource.id || !confirm(`Delete resource "${resource.name ?? 'Resource'}"?`))
    return
  ...
  <UDashboardPanel id="standalone-resources">
    ...
    <template #body>
      <div class="space-y-3">
            ...
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p class="mt-1 text-xs text-(--ui-text-muted)">
                  Create and manage redeemable resources for this workshop.
                </p>
              </div>
              <UButton
                icon="i-lucide-plus"
          ...
          >
            ...
            >
              <div class="min-w-0">
                ...
                <p
                  v-if="resource.redemptionStmt"
                  class="mt-1 text-xs text-(--ui-text-muted)"
                >
                  Statement: {{ resource.redemptionStmt }}
                </p>
              </div>
              <div class="flex flex-wrap gap-2">
                ...
                <UButton
                  ...
                  :loading="deleteResourceMutation.isPending.value"
        ...
        <UModal v-model:open="isResourceModalOpen">
          <template #content>
            <UCard>
              <template #header>
                <div class="flex items-center justify-between">
              ...
              >
                ...
                <UFormField label="Redemption Statement">
                  <UTextarea
                    v-model="resourceForm.redemptionStmt"
                    autoresize
                    placeholder="Optional statement shown when redeeming this resource"
                  />
                </UFormField>
                <label class="flex items-center gap-2 text-sm">
                  <UCheckbox v-model="resourceForm.isPublished" />
                  Published
                </label>
                <div class="flex justify-end gap-2">
                  ...
                  <UButton
                    type="submit"
                    :loading="createResourceMutation.isPending.value || updateResourceMutation.isPending.value"
                  >
                    {{ editingResourceId ? 'Save' : 'Create' }}
                  </UButton>
                </div>
              </form>
            </UCard>
